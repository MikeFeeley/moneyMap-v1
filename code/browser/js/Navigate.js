class Navigate {
  ...
  async _getData (
    ...
    return {
      ...
      subnote:   note,
      groups:    groups,
      startBal: this._actuals.getStartingCashFlowBalance (dates [0].start),
  ...
  async _getYearsData (type, id, blackouts) {
    var getBlackouts = async (d) => {
      const boAmts = await Promise .all (blackouts .map (async b => (await this._getAmounts (type, b._id, this._budget .isCredit (b), dates)) [0]));
    ...
    const dates = [{start: this._budget .getStartDate(), end: this._budget .getEndDate()}];
  ...
  _addBigPicture() {
    ...
    let update = varianceList => {
      ...
      if (ovr < -50)
        ...
        ovr = 0;
      let stc = this._actuals.getStartingCashFlowBalance ();
      if (stc < -50 || stc > 50) {
        let tt =
            'End of year ' +
  ...
  async _getHistoryData (parentIds, date, filter) {
    ...
    result.startBal = [this._actuals.getStartingCashFlowBalance () - historicAmounts.reduce ((t, year) => {
