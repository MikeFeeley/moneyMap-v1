class BudgetYearGraph {
  ...
  _getData() {
    var data = [];
    for (let genere of [this._budget .getIncomeCategory(), this._budget .getSavingsCategory(), this._budget .getExpenseCategory()]) {
      data .push ({
        name:   genere .name,
  ...
  _setDataset() {
    ...
    var datasets = data .map ((d,i) => {
      return {
        label:           d .name,
        data:            [i==0? norm [i]:  0, i==0? 0: norm [i]],
        values:          [i==0? d .amount: 0, i==0? 0: d.amount],
